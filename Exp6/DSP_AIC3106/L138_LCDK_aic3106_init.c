...
*EDMA3_PaRAM_ptr++ = (int32_t)((((DMA_PP_BUFFSIZE)<<16) & 0xFFFF0000) | 0x00000004);
 // BCNT = DMA_PP_BUFFSIZE(arrays per frame), ACNT = 4(bytes per array) writing to a (32-bit) McASP0 serialiser
...
// WARNING - this assumes that adc_gain is a sensible value
  ...
  {
    AIC3106_writeRegister( 19, 0x7C );  // power up L ADC and do not connect LINE1L to L ADC
    AIC3106_writeRegister( 22, 0x7C );  // power up R ADC and do not  connect LINE1R to R ADC
    AIC3106_writeRegister( 21, 0x00 );  // connect LINE1R to L ADC to correct LCDK pcb swapped channels routing from inputs in J10
    AIC3106_writeRegister( 24, 0x00 );  // connect LINE1L to R ADC to correct LCDK pcb swapped channels routing from inputs in J10
  }
  else
  {
    AIC3106_writeRegister( 19, 0x7C );  // power up L ADC and do not connect LINE1L to ADC
    AIC3106_writeRegister( 22, 0x7C );  // power up R ADC and do not connect LINE1R to ADC
    AIC3106_writeRegister( 17, 0x0F );  // 17 MIC3L is connected to L ADC (default 0xFF, NC) 0x00 0dB
    AIC3106_writeRegister( 18, 0xF0 );  // 18 MIC3R is connected to R ADC (default 0xFF, NC) 0x00 0dB
...
// WARNING - this assumes that dac_atten is a sensible value
  ...
  AIC3106_writeRegister( 85, 0x80 );  // DAC_R1 to LEFT_LOP/M Volume Routed, Gain=0dB. This connection swaps the bad routing of output in LCDK J10.
  AIC3106_writeRegister( 86, 0x09 );  // LEFT_LOP/M Output Mute=OFF bit3 bit0 is REad only ?! must be set!
  AIC3106_writeRegister( 89, 0x80 );  // DAC_L1 to RIGHT_LOP/M Volume Routed, Gain=0dB. This connection swaps the bad routing of output in LCDK J10.
...
void L138_initialise_edma(int32_t fs, int16_t adc_gain, int16_t dac_atten, int8_t input_type)
{
  ...
  pingOUT = (int16_t *)malloc(DMA_PP_BUFFSIZE*sizeof(uint32_t));
  pongOUT = (int16_t *)malloc(DMA_PP_BUFFSIZE*sizeof(uint32_t));
  pingIN = (int16_t *)malloc(DMA_PP_BUFFSIZE*sizeof(uint32_t));
  pongIN = (int16_t *)malloc(DMA_PP_BUFFSIZE*sizeof(uint32_t));
